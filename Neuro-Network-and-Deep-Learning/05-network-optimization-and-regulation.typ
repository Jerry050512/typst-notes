...
- 必要性：如果不同特征的取值范围（尺度）相差巨大，例如一个特征在 [0, 10] 区间，另一个在 [0, 1] 区间，那么在训练如 $y = tanh(w_1 dot x_1 + w_2 dot x_2 + b)$ 这样的网络层时，优化器需要花费更多精力来平衡不同权重 w1 和 w2 的学习速率。由于tanh等激活函数在其输入绝对值较大时会进入饱和区，如果 w1*x1 + w2*x2 + b 的值过大或过小，都会导致激活函数的导数趋近于0，从而引发梯度消失问题，严重影响训练效率。
...
权重衰减是在每次参数更新时，按照一个衰减系数 β 对参数 θ 进行缩减，其更新公式为：$θ_t ← (1 - β)θ_{t-1} - alpha g_t$。
...
#table(
  ...
  [*知识点*], [*选择题*], [*简答题*], [*综合计算题*],
  [鞍点 vs. 局部最小], [#emoji.checkmark], [#emoji.checkmark], [],
  [平坦 vs. 尖锐最小值], [#emoji.checkmark], [], [],
  [L1/L2 正则化区别], [#emoji.checkmark], [#emoji.checkmark], [],
  [权重衰减], [#emoji.checkmark], [], [#emoji.checkmark],
  [丢弃法 (Dropout) 原理], [], [#emoji.checkmark], [],
  [逐层归一化], [#emoji.checkmark], [#emoji.checkmark], [],
  [标签平滑], [#emoji.checkmark], [], [#emoji.checkmark],
)
