...
#figure(
  table(
    ...
    [*功能*], [全局中断使能], [保留], [保留], [串行口中断使能], [定时器1中断使能], [外部中断1使能], [定时器0中断使能], [外部中断0使能],
  ),
...
#figure(
  table(
    ...
    [*功能*], [保留], [保留], [保留], [串行口中断优先级], [定时器1中断优先级], [外部中断1优先级], [定时器0中断优先级], [外部中断0优先级], 
  ),
...
(注意：`TR0` 和 `TR1` 是定时器运行控制位，不是中断标志。查询方式时,要用软件清零.)
...
-   `SM1` (SCON.6): 串行口工作模式选择位1。
    ...
    #table(
      ...
      align: center,
      [*SM0*], [*SM1*], [*模式*], [*说明*], [*波特率*],
      [0], [0], [0], [8位同步移位寄存器], [固定 ($f_"osc"/12$)],
      [0], [1], [1], [8位UART，波特率可变 (由定时器1决定)], [可变],
      [1], [0], [2], [9位UART，波特率固定 ($f_"osc"/32$ 或 $f_"osc"/64$)], [固定],
