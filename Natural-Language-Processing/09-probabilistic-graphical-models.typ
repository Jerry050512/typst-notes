...
表示：在时刻 $t$ 处于状态 $q_i$，且前 $t$ 个观测已经出现的联合概率。记前向概率向量 $alpha_t = vec(alpha_t(1), alpha_t(2), dots.v, alpha_t(N))$。
...
#example(title: [前向算法计算示例])[
  沿用三个盒子的例子，$b("红") = vec(0.5, 0.4, 0.7)$，$b("白") = vec(0.5, 0.6, 0.3)$，观测序列 $O = ("红", "白", "红")$，$T=3$。

  1. *初始化* ($t=1$，观测“红”)
  $ alpha_1 = "diag"(pi) b("红") = mat(0.2, 0, 0; 0, 0.4, 0; 0, 0, 0.4) vec(0.5, 0.4, 0.7) = vec(0.10, 0.16, 0.28) $

  2. *递推到 $t=2$* (观测“白”)
  $ A^T alpha_1 = mat(0.5, 0.3, 0.2; 0.2, 0.5, 0.3; 0.3, 0.2, 0.5) vec(0.10, 0.16, 0.28) = vec(0.154, 0.184, 0.202) $
  $ alpha_2 = "diag"[b("白")] vec(0.154, 0.184, 0.202) = mat(0.5, 0, 0; 0, 0.6, 0; 0, 0, 0.3) vec(0.154, 0.184, 0.202) = vec(0.077, 0.1104, 0.0606) $

  3. *递推到 $t=3$* (观测“红”)
  $ A^T alpha_2 = mat(0.5, 0.3, 0.2; 0.2, 0.5, 0.3; 0.3, 0.2, 0.5) vec(0.077, 0.1104, 0.0606) = vec(0.08374, 0.08878, 0.07548) $
  $ alpha_3 = "diag"[b("红")] vec(0.08374, 0.08878, 0.07548) = mat(0.5, 0, 0; 0, 0.4, 0; 0, 0, 0.7) vec(0.08374, 0.08878, 0.07548) = vec(0.04187, 0.035512, 0.052836) $
...
表示：已知时刻 $t$ 处于状态 $q_i$，从 $t+1$ 到 $T$ 的剩余观测序列出现的概率。记后向概率向量 $beta_t = vec(beta_t(1), dots.v, beta_t(N))$。
...
#example(title: [后向算法计算示例])[
  ...
  2. *递推到 $t=2$* (下一观测“红”)
  $ "diag"[b("红")] beta_3 = mat(0.5, 0, 0; 0, 0.4, 0; 0, 0, 0.7) vec(1, 1, 1) = vec(0.5, 0.4, 0.7) $
  $ beta_2 = A vec(0.5, 0.4, 0.7) = mat(0.5, 0.2, 0.3; 0.3, 0.5, 0.2; 0.2, 0.3, 0.5) vec(0.5, 0.4, 0.7) = vec(0.54, 0.49, 0.57) $

  3. *递推到 $t=1$* (下一观测“白”)
  $ "diag"[b("白")] beta_2 = mat(0.5, 0, 0; 0, 0.6, 0; 0, 0, 0.3) vec(0.54, 0.49, 0.57) = vec(0.27, 0.294, 0.171) $
  $ beta_1 = A vec(0.27, 0.294, 0.171) = mat(0.5, 0.2, 0.3; 0.3, 0.5, 0.2; 0.2, 0.3, 0.5) vec(0.27, 0.294, 0.171) = vec(0.2451, 0.2622, 0.2277) $

  4. *终止*
  $ "diag"[b("红")] beta_1 = mat(0.5, 0, 0; 0, 0.4, 0; 0, 0, 0.7) vec(0.2451, 0.2622, 0.2277) = vec(0.12255, 0.10488, 0.15939) $
  $ P(O | lambda) = pi^T vec(0.12255, 0.10488, 0.15939) = 0.2 times 0.12255 + 0.4 times 0.10488 + 0.4 times 0.15939 = 0.130218 $
...
记 $delta_t = vec(delta_t(1), dots.v, delta_t(N))$，维特比算法的矩阵形式如下：
...
#example(title: [Viterbi 算法计算示例])[
  ...
  1. *初始化* ($t=1$，观测“红”)
  $ delta_1 = "diag"(pi) b("红") = mat(0.2, 0, 0; 0, 0.4, 0; 0, 0, 0.4) vec(0.5, 0.4, 0.7) = vec(0.10, 0.16, 0.28) $
  $ psi_1 = vec(0, 0, 0) $
  ...
  $
  $ "rowmax"[A^T "diag"(delta_1)] = vec(#viterbi-max[0.056], #viterbi-max[0.084], #viterbi-max[0.140]), quad psi_2 = vec(3, 3, 3) $
  $ delta_2 = "diag"[b("白")] vec(0.056, 0.084, 0.140) = vec(0.0280, 0.0504, 0.0420) $
  ...
  $
  $ "rowmax"[A^T "diag"(delta_2)] = vec(#viterbi-max[0.01512], #viterbi-max[0.02520], #viterbi-max[0.02100]), quad psi_3 = vec(2, 2, 3) $
  $ delta_3 = "diag"[b("红")] vec(0.01512, 0.02520, 0.02100) = vec(0.007560, 0.010080, 0.014700) $
  ...
  $
  $ "rowmax"[A^T "diag"(delta_3)] = vec(#viterbi-max[0.0037800], #viterbi-max[0.0050400], #viterbi-max[0.0073500]), quad psi_4 = vec(1, 2, 3) $
  $ delta_4 = "diag"[b("白")] vec(0.0037800, 0.0050400, 0.0073500) = vec(0.00189000, 0.00302400, 0.00220500) $
...
#example(title: [CRF 矩阵计算])[
  ...
  *计算所有路径的非规范化概率*：
  ...
  这里以路径$(y_0=1, y_1=2, y_2=1, y_3=2, y_4=1)$为例介绍怎么计算: 

  $
    y(1, 2, 1, 2, 1) &= M_1 (y_0, y_1) times M_2 (y_1, y_2) times M_3 (y_2, y_3) times M_4 (y_3, y_4) \
    &= M_1 (1,2) times M_2 (2,1) times M_3 (1, 2) times M_4 (2, 1) \
    &= 0.5 times 0.7 times 0.4 times 1 \
    &= 0.14
  $
  ...
  *归一化因子*：
  ...
  或通过矩阵乘法：
  $ M_1 M_2 M_3 M_4 = mat(0.5, 0.5; 0, 0)mat(0.3, 0.7; 0.7, 0.3)mat(0.5, 0.4; 0.6, 0.4)mat(1, 0; 1, 0) = mat(0.95, 0; 0, 0) = 0.95 $
