...
- `RS1, RS0 (PSW.4 / PSW.3)`: 工作寄存器组选择位。通过这两位可以选择4组工作寄存器 (R0-R7) 中的当前活动组。
  #table(columns: 3, align: center, header_rows: 1, [*RS1*], [*RS0*], [*选定寄存器组及地址范围*], [0], [0], [第0组 (00H-07H)], [0], [1], [第1组 (08H-0FH)], [1], [0], [第2组 (10H-17H)], [1], [1], [第3组 (18H-1FH)])
...
-   图 #figure(image("assets/storage-config.png"), caption: [程序存储器空间配置总结]) 展示了MCS-51的整体存储器映射，包括内部和外部程序/数据存储区。外部扩展时，地址译码决定了外部芯片在这些区域的具体位置。
...
=== 3. 定时器/计数器控制寄存器 `TCON` (Timer Control)
...
#figure(
  table(
    ...
    [*对应定时器*], colspan: 4,[Timer 1], colspan: 4,[Timer 0],
...
2.  *所需计数值计算*:
    目标延时 = $1 "ms" = 1000 "µs"$。
    所需计数值 N = $1000 "µs" / (1.085 "µs/count") approx 921.6 " counts"$. 更精确的计算应避免过早的近似，直接使用分数： $N = 1000 \text{µs} \times (11.0592 \text{MHz} / 12) = 1000 \times 10^{-6} \times (11.0592 \times 10^6 / 12) = 1000 \times 11.0592 / 12 = 921.6$. 实际编程中，通常选择一个晶振频率使得每个机器周期的时间更容易计算（如12MHz，1µs/机器周期）。
...
=== 2. 电源控制寄存器 `PCON` (Power Control) - SMOD位
...
-   波特率：由定时器1的溢出率决定。
    公式: $text(Baud Rate) = (2^SMOD / 32) \times text(Timer 1 Overflow Rate)$
    当定时器1工作在模式2 (8位自动重载模式) 时，Timer 1 Overflow Rate = $f_("osc") / (12 \times (256 - TH1))$
    所以， $text(Baud Rate) = (2^SMOD / 32) \times (f_("osc") / (12 \times (256 - TH1)))$
    由此可得 `TH1` 的计算公式: $TH1 = 256 - ( (2^SMOD \times f_("osc")) / (384 \times text(Baud Rate)) )$
...
2.  计算 `TH1` 值:
    $TH1 = 256 - ( (2^1 \times 11.0592 \times 10^6) / (384 \times 9600) )$
    $TH1 = 256 - ( (2 \times 11059200) / 3686400 )$
...
== 1. 模拟-数字转换器 (ADC - Analog-to-Digital Converter)
...
== 2. 数字-模拟转换器 (DAC - Digital-to-Analog Converter)
...
== 3. 简单7段数码管驱动 (Single Digit 7-Segment Display)
