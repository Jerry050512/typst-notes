...
$
Y = vec(y_1, dots.v, y_n), quad
X = mat(x_1, 1; dots.v, dots.v; x_n, 1), quad
B = vec(m, b)
$
...
#formula[
  $
  E &= ||Y - X B||^2 = (Y - X B)^T (Y - X B) \
    &= Y^T Y - 2 (X B)^T Y + (X B)^T (X B)
  $
]
...
为克服垂直最小二乘法的缺陷，采用 *点到直线的垂直距离* 作为误差度量。

直线表示为：$a x + b y = d$，其中 $a^2 + b^2 = 1$（单位法向量 $N = (a, b)$）

点 $(x_i, y_i)$ 到直线的距离为 $|a x_i + b y_i - d|$。

*目标*：找到 $(a, b, d)$ 最小化垂直距离平方和
...
#methodblock[
  *步骤 1：对 $d$ 求偏导*
  ...
  $
  (partial E) / (partial d) = -2 sum_(i=1)^n (a x_i + b y_i - d) = 0
  $

  得到：
  $
  d = a dot 1/n sum_(i=1)^n x_i + b dot 1/n sum_(i=1)^n y_i = a overline(x) + b overline(y)
  $

  即直线过数据点的 #key[质心] $(overline(x), overline(y))$。

  *步骤 2：代回原式*

  $
  E = sum_(i=1)^n (a(x_i - overline(x)) + b(y_i - overline(y)))^2 = ||U N||^2 = (U N)^T (U N)
  $
  ...
  *步骤 4：求解 $(U^T U) N = 0$（在约束 $||N||^2 = 1$ 下）*

  解为 $U^T U$ 的 #key[最小特征值对应的特征向量]（即齐次线性系统 $U N = 0$ 的最小二乘解）。
]
...
其中：
- $rho$：原点到直线的垂直距离（$rho gt.eq 0$）
- $theta$：直线法向量与 $x$ 轴的夹角（$theta in [0, 180 degree]$）
  ...
  *输出*：直线的个数以及直线的极坐标参数
  ...
  #methodblock[
    *算法步骤*：
    ...
    + 将 $theta in [0, 180 degree]$，$rho in [0, rho_max]$ 所确定的矩形区域按一定步长划分成小格，并将每个小格的累加器 $H(rho, theta)$ 初始化为零

    + 对于每个数据点 $(x_i, y_i)$，进行以下循环操作：

    4. #h(1em) 在 $[0, 180 degree]$ 范围内遍历所有的 $theta$，进行以下循环操作：
    ...
    + 找到 $H(rho, theta)$ 的 #key[局部极值点]，每个极值点对应一条直线
...
- 圆心必沿梯度方向（指向圆心或反方向）
- 候选圆心：$(x, y) + r dot nabla I(x, y)$ 或 $(x, y) - r dot nabla I(x, y)$
- 随半径变化，候选点形成两条相交直线
