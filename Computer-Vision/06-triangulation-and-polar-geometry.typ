...
/ 三角化: 拍一张照片只有二维坐标。但如果有两台相机从不同角度拍同一点，两条射线会在三维空间中相交——这个交点就是三维坐标。这就是 #key[三角化]。

== 对极几何核心概念

#figure(
  image("assets/06-epipolar-geometry.png", width: 50%),
  caption: [极几何示意图\* (此处考察概念和画图)],
)
...
/ 本质矩阵 (Essential Matrix): 对规范化摄像机拍摄的两个视点图像间的极几何关系进行代数描述，它编码了两个摄像机间的旋转和平移关系。 $ E = [bold(t)]_times R $
/ 基础矩阵 (Fundamental Matrix): 对一般摄像机拍摄的两个视点图像间的极几何关系进行代数描述。 $ F = K'^(-T) E K^(-1) $
/ 单应矩阵 (Homography Matrix): 描述空间平面在两个摄像机下的投影变换关系，适用于平面场景。
