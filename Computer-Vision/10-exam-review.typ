...
#methodblock[
  *算法步骤*：
  ...
  2. 将$theta in [0, 180 degree)$，$rho in [-rho_max, rho_max]$所确定的矩形区域按一定步长划分成小格，并将每个小格的累加器$H(rho, theta)$初始化为零
  3. 对于每个数据点$(x_i, y_i)$，进行以下循环操作：
     - 在$[0, 180 degree)$范围内遍历所有离散的$theta$
     - 计算$rho = x_i cos theta + y_i sin theta$
     - $H(rho, theta) = H(rho, theta) + 1$
  4. 找到$H(rho, theta)$超过阈值的局部最大值，每个峰值对应一条候选直线
  5. 输出直线个数以及每条直线的参数
]
...
#methodblock[
  ...
  *拍摄要求*：一般五内参情况至少3个不同角度/位置的图像；若预先固定skew为零，理论最少可为2幅，实践中仍应使用更多姿态
...
#table(
  ...
  [编码信息], [旋转$R$和平移方向$bold(t)$], [像素坐标下的两视图对极几何],
...
#table(
  ...
  [曝光方式], [所有像素同时开始和结束曝光；数据可随后逐行读出], [逐行顺序曝光],
  [运动失真], [无卷帘几何失真，但仍可能运动模糊], [可能有果冻效应（Jelly Effect）],
  [适用场景], [高速运动拍摄], [静态或低速场景],
  [成本], [较高], [较低],
)
...
#table(
  ...
  [物方远心], [孔径光阑在像方焦平面], [在规定远心范围内，放大倍率对物距变化不敏感],
  [像方远心], [孔径光阑在物方焦平面], [在规定范围内，放大倍率对传感器位置偏差不敏感],
  [双侧远心], [同时具备物方和像方远心], [同时降低物体和传感器位置变化造成的倍率误差],
)
