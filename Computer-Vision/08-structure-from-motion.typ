...
#key[\*考试重点：SfM全称（必考）、增量法求SfM（必考简答）]

/ 运动恢复结构 (Structure from Motion, SfM): #key[\*必考全称] 从场景的多张图像中，同时恢复三维结构（3D点坐标）和相机运动（相机位姿）。

#warnbox[
  #key[\*必考术语]：Structure from Motion (SfM) - 运动恢复结构

  考试可能要求：
  - 写出SfM的全称（英文）
  - 解释SfM的含义
  - 说明SfM的目标：从2D图像恢复3D结构和相机位姿
]
...
当已知相机内参 $K$ 时，问题变为求解相机外参（旋转 $R$ 和相对平移方向 $T$）以及 3D 点坐标；仅凭双目对应关系，平移和场景的绝对尺度不可确定。
...
#methodblock[
  ...
  *步骤 3：分解本质矩阵 $E arrow.r R, T$*
  ...
  将 $E$ 分解为旋转 $R$ 和平移方向 $T$（详见下节）。

  *步骤 4：三角化重建 3D 点*

  选定 $T$ 的尺度后，对每对对应点进行三角化求解同一尺度下的 $X_j$。
]
...
本质矩阵 $E = [bold(T)]_times R$ 编码旋转和平移方向，需要分解出 $R$ 和只确定到尺度的 $T$。
...
#formula[
  $
  min_(M, X) sum_((i,j) in Omega) D(x_(i j), pi(M_i, X_j))^2
  $
]

其中 $Omega$ 是实际存在二维观测的相机—点索引集合，$pi$ 表示包含齐次归一化的投影函数，$D(dot, dot)$ 是像素距离。不能对不可见、没有观测的 $(i,j)$ 项求误差。
...
#methodblock[
  ...
  *步骤 2：求解线段长度*
  ...
  求解该方程组可得到 #key[最多 4 组可能解]，通常用第 4 个点按重投影误差验证消歧义。
...
#warnbox[
  #key[\*考试要点：增量法求SfM（简答题）]

  必须掌握的要点：
  1. #key[预处理阶段]：特征提取 → 特征匹配 → 几何验证（RANSAC估计F或H矩阵）
  2. #key[初始化]：选择初始图像对 → 估计本质矩阵E → 分解得R,T → 三角化初始点云
  3. #key[增量重建]：迭代添加新视图
     - 选择能看到最多已重建3D点的新视图
     - 用PnP方法估计新视图的相机位姿
     - 三角化新的3D点
     - 运行Bundle Adjustment全局优化
  4. #key[结束条件]：所有视图都已添加到重建中

  关键算法：特征匹配、RANSAC、八点法、本质矩阵分解、三角化、PnP、Bundle Adjustment
]
