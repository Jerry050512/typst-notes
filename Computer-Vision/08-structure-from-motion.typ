...
将上面预处理与增量重建两个阶段的步骤串成整体：

#methodblock[
  ```
  输入：图像集 {I_1, …, I_m}、相机内参 K
  输出：相机位姿 {(R_i, T_i)}、3D 点云 {X_j}

  # ── 预处理 ──
  对每张图像 I_i：
      提取特征点并计算描述符（SIFT / ORB）
  对每对图像 (I_i, I_j)：
      特征匹配（最近邻距离比）
      用 RANSAC 估计 F（或 H），过滤误匹配
  将匹配点连接成轨迹 tracks
  构建连通图 G（节点 = 图像，边 = 匹配点数足够的图像对）

  # ── 初始化 ──
  从 G 中选择初始边 e（匹配点多、基线适中）
  用 RANSAC + 八点法估计 E，分解得 (R_1, T_1)、(R_2, T_2)
  三角化 e 中可见的轨迹，得到初始 3D 点云
  从 G 中删除边 e

  # ── 增量扩展 ──
  当 G 中还有边时，重复：
      选择边 e，使 track_e ∩ {已重建 3D 点} 最多
      用 PnP 估计该视图位姿 (R, T)
      三角化新轨迹，扩展 3D 点云
      从 G 中删除边 e
      执行 Bundle Adjustment 联合优化所有相机与 3D 点
  结束
